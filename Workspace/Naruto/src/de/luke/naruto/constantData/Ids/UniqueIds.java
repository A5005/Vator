package de.luke.naruto.constantData.Ids;

public class UniqueIds {

	public static final int NoneMat = 1;
	
	//MaterialGroupIcons
	public static final int CommonMat = 2;
	public static final int UnCommonMat = 3;
	public static final int RareMat = 4;
	public static final int EpicMat = 5;
	
	//WeaponGroupIcons
	public static final int CommonWeap = 6;
	public static final int UnCommonWeap = 7;
	public static final int RareWeap = 8;
	public static final int LegendWeap = 9;
	public static final int ExclusiveWeap = 10;

	//CommonMat
	public static final int Stick = 11;
	public static final int WoodenBtn = 12;
	public static final int StoneBtn = 13;
	public static final int WoodenPresPlate = 14;
	public static final int StonePresPlate = 15;
	public static final int Feather = 16;
	public static final int Leather = 17;
	public static final int Paper = 18;
	public static final int Book = 19;
	
	//UnCommonMat
	public static final int String = 20;
	public static final int Bone = 21;
	public static final int Brick = 22;
	public static final int NeBrick = 23;
	public static final int Clay = 24;
	public static final int Coal = 25;
	public static final int Flint = 26;
	public static final int Creeper = 27;
	public static final int Compass = 28;
	
	//RareMat
	public static final int Spider = 29;
	public static final int Glowstone = 30;
	public static final int Quartz = 31;
	public static final int Cream = 32;
	public static final int Iron = 33;
	public static final int Gold = 34;
	public static final int PrisShard = 35;
	public static final int PrisCrystal = 36;
	public static final int Watch = 37;
	
	//EpicMat
	public static final int Ferm = 38;
	public static final int Tear = 39;
	public static final int Blaze = 40;
	public static final int Slime = 41;
	public static final int Ender = 42;
	public static final int Diamond = 43;
	public static final int Emerald = 44;
	public static final int Bottle = 45;
	public static final int Star = 46;
	
	
	//CommonWeap
	public static final int Arrow = 50;
	public static final int WoodAxe = 51;
	
	//UnCommonWeap
	public static final int StoneAxe = 60;	
	
	//RareWeap
	public static final int IronAxe = 70;
	
	//ExclusiveWeap
	public static final int DiamondAxe = 80;
	

	//Fill
	public static final int Glass = 100;
	
	//Crafting
	public static final int Workbench = 200;
	public static final int GreenWool = 201;
	public static final int Barrier = 202;
	
	
}
